#ifndef REBECCA_FRAMEWORK_ID_H
#define REBECCA_FRAMEWORK_ID_H
 ...
 */
class REBECCA_EXPORT Id : public InnerTemplate
{
	public:
		...
		/**
		 * Calls GraphBuilderFramework::getId() and
		 * returns the result
		 *
		 * Returns GraphBuilderFramework::getId()
		 *
		 * @exception InternalProgrammerErrorException is thrown 
		 * only if the error is so grave that the entire AIML 
		 * engine has to be shut down.
...
using rebecca::framework::impl::Id;
