#ifndef REBECCA_FRAMEWORK_LEARN_H
#define REBECCA_FRAMEWORK_LEARN_H
 ...
 */
class REBECCA_EXPORT Learn : public InnerTemplateListImpl
{
	public:
		 ...
		 * then calls GraphBuilderFramework.addFile() with
		 * the string.  It <b>DOES NOT</b> call 
		 * GraphBuilderFramework::createGraph().  That would
		 * invalidate internal iterators and cause the 
		 * Rebecca engine to crash.
		 * 
		 * Instead it relys on GraphBuilder::getResponse() to
		 * call GraphBuilder::createGraph() for it.
		 * 
		 * @return The string after calling InnerTemplateListImpl::getString(),
		 * in this case an empy string.
		 */
...
using rebecca::framework::impl::Learn;
