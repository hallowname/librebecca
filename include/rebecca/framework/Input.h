#ifndef REBECCA_FRAMEWORK_INPUT_H
#define REBECCA_FRAMEWORK_INPUT_H
 ...
 */
class REBECCA_EXPORT Input : public InnerTemplate
{
	public:
		...
		/**
		 * Calls GraphBuilderFramework::getPreviousUserInput() with the
		 * index attribute set through Input::setAttribute() 
		 * and returns the result.
		 * 
		 * @return the result of calling GraphBuilderFramework::getPreviousUserInput()
		 * with the index attribute set through Input::setAttribute()
		 *
		 * @exception InternalProgrammerErrorException is thrown 
		 * only if the error is so grave that the entire AIML 
		 * engine has to be shut down.
...
using rebecca::framework::impl::Input;
