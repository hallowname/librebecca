#ifndef REBECCA_FRAMEWORK_TEMPLATESIDETHAT_H
#define REBECCA_FRAMEWORK_TEMPLATESIDETHAT_H
 ...
 */
class REBECCA_EXPORT TemplateSideThat : public InnerTemplate
{
	public:
		...
		/**
		 * Calls GraphBuilderFramework::getPreviousBotResponsePunctuation() 
		 * with the index attribute set through TemplateSideThat::setAttribute() 
		 * and returns the result
		 * 
		 * @return the result of calling 
		 * GraphBuilderFramework::getPreviousBotResponsePunctuation()
		 * with the name attribute set through TemplateSideThat::setAttribute()
		 *
		 * @exception InternalProgrammerErrorException is thrown 
		 * only if the error is so grave that the entire AIML 
		 * engine has to be shut down.
	...
	private:
		...
		* idiom on the internet for more information about this.
		*/
		TemplateSideThatImpl *m_pimpl;
};

} //end of namespace impl

//Expose just the class name to the framework namespace
using rebecca::framework::impl::TemplateSideThat;
