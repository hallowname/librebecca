#ifndef REBECCA_FRAMEWORK_STAR_H
#define REBECCA_FRAMEWORK_STAR_H
 ...
 */
class REBECCA_EXPORT Star : public InnerTemplate
{
	public:
		...
		/**
		 * Calls GraphBuilderFramework::getStar() with the index
		 * set from Star::setAttribute() and returns the result.
		 *
		 * @return The result from calling GraphBuilderFramework::getStar()
		 * with the index set from Star::setAttribute()
		 *
		 * @exception InternalProgrammerErrorException is thrown 
		 * only if the error is so grave that the entire AIML 
		 * engine has to be shut down.
...
using rebecca::framework::impl::Star;
