#ifndef REBECCA_FRAMEWORK_BOT_H
#define REBECCA_FRAMEWORK_BOT_H
 ...
 */
class REBECCA_EXPORT Bot : public InnerTemplate
{
	...
	public:
		...
		/**
		 * Calls GraphBuilderFramework::getBotPredicate() with the
		 * name attribute set through Bot::setAttribute() and returns
		 * the result
		 * 
		 * @return the result of calling GraphBuilderFramework::getBotPredicate()
		 * with the name attribute set through Bot::setAttribute()
		 *
		 * @exception InternalProgrammerErrorException is thrown 
		 * only if the error is so grave that the entire AIML 
		 * engine has to be shut down.
...
using rebecca::framework::impl::Bot;
