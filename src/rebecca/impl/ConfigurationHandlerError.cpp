...
void ConfigurationHandlerError::warning(const SAXParseException &exc)
{
	try
	{
		...
		IdentificationManager *ident = IdentificationManager::getInstance();
		GraphBuilderFramework *builder = factory->getGraphBuilderFramework();
		GraphBuilderAIML::getInstance()->getCallBacks().XMLParseWarning(msg.c_str(), ident->getUserId(), ident->getBotId(), ident->getEndUserId());
	}
	catch(bad_lexical_cast &)
	{
		throw InternalProgrammerErrorExceptionImpl("bad lexical cast within SAXParseException");
...
void ConfigurationHandlerError::error(const SAXParseException &exc)
{
	...
	try
	{
		...
		IdentificationManager *ident = IdentificationManager::getInstance();
		GraphBuilderFramework *builder = factory->getGraphBuilderFramework();
		GraphBuilderAIML::getInstance()->getCallBacks().XMLParseError(msg.c_str(), ident->getUserId(), ident->getBotId(), ident->getEndUserId());
	}
	catch(bad_lexical_cast &)
	{
		throw InternalProgrammerErrorExceptionImpl("bad lexical cast within SAXParseException");
